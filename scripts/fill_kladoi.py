...
# pinakes = session.query(Pinakas).filter(Pinakas.sxoliko_etos_id == 15, Pinakas.kathgoria_id == 3).all()
pinakes = session.query(Pinakas).filter(Pinakas.sxoliko_etos_id == 15, Pinakas.klados_id != '254').all()
for pinakas in pinakes:
    klados_id = pinakas.klados_id

    filename = pinakas.lektiko_pinaka
    full_path = Path(data_path, pinakas.path_pinaka)
    full_filename = full_path.joinpath(filename)
    csv_temp_file_path = full_path.joinpath(filename[:-3])

    print(full_filename)
    print(csv_temp_file_path)

    #gz to cvs
    with gzip.open(str(full_filename), 'rb') as infile:
